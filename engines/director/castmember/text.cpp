...
TextCastMember::TextCastMember(Cast *cast, uint16 castId, Common::SeekableReadStreamEndian &stream, uint16 version, uint8 flags1, bool asButton)
	...
	// _bgcolor is resolved lazily in getBackColor() against the palette that is
	// active when the field is drawn, not the one present at cast-load time (a
	// movie may install its own CLUT afterwards). We keep only _bgpalinfo here.
...
uint32 TextCastMember::getBackColor() {
	// Resolve the background against the palette that is active now, not the one
	// present when the cast loaded. A movie may install its own CLUT after the
	// casts are loaded (e.g. the LOG_IN name fields in "Ein Fall fuer Muetze &
	// Co", whose blue-gray bgpalinfo would otherwise render as a stale green
	// index while the surrounding palette-indexed BITD bitmaps look correct).
	// Skip re-deriving once Lingo has explicitly set a colour.
	if (!_bgColorSet)
