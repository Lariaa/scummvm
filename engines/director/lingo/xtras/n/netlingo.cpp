...
// netDone/netError/netTextResult are polled in a wait-loop after a network
// request (e.g. the "e-mail versenden" feature in Loewenzahn 5/6):
//   if netDone(netID) = 0 then go(the frame) else if netError(netID) = "OK" ...
// We don't perform real networking, so report the operation as immediately
// finished ("done" = 1) and report "OK" rather than 0 to avoid an infinite
// wait-loop and to keep the error-string comparisons well-typed.
