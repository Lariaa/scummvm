...
/*
 * Buddy API's Mac companion. Per its own message table this build "contains
 * dummy Buddy API Mac functions for Windows-based authoring": the Windows
 * Xtra exists purely so that scripts written against the Mac API still load
 * and run, with every call a no-op. Stubbing it out therefore matches what
 * the real Xtra does on this platform, rather than approximating it.
 *
 * TKKG 6 ships it next to the real BudAPI (see budapi.cpp) but never calls
 * into it; registering it just stops the "Unimplemented xlib" warning.
 */
