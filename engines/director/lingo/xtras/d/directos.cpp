...
/*
 * Of the 90 methods below only the file queries are answered for real; the rest
 * warn and return 0. TKKG 7's photo album calls dosIsFileExist, dosCopyFile,
 * dosDeleteFile and dosGetFileTime when it saves or prints a picture.
 *
 * The msgTable carries no return types and the Xtra ships no error strings -- it
 * reports failures through GetLastError, which dosGetLastError hands back. So the
 * exact codes of the mutating calls are unknown, and copying or deleting files in
 * the game directory is nothing we want to do anyway: they stay stubs, the same
 * way FileXtra4 implements fx_FileExists but leaves fx_FileCopy/fx_FileDelete
 * alone (see filextra4.cpp).
 */
