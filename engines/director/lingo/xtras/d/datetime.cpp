...
// These global handlers return the current local date/time. Games use them for
// e.g. a live in-game analog clock (GetHours/GetMinutes drive the hand frames)
// and date checks (GetYear). Implemented for real against the host clock so the
// clock ticks instead of being frozen at 0:00.
...
void DateTimeXtra::m_GetMonth(int nargs) {
	...
	TimeDate td;
	g_system->getTimeAndDate(td);
	// Director months are 1-12; TimeDate::tm_mon is 0-11.
...
void DateTimeXtra::m_GetYear(int nargs) {
	...
	TimeDate td;
	g_system->getTimeAndDate(td);
	// TimeDate::tm_year is years since 1900; the Xtra returns the full year.
...
void DateTimeXtra::m_GetWeekDay(int nargs) {
	...
	TimeDate td;
	g_system->getTimeAndDate(td);
	// 0 = Sunday, matching both the Xtra and TimeDate::tm_wday.
